function [ totalCost ] = randomizedPlannedControllerPlacement( controllers, mat, n, m )
...
%                     controllerLoad (1,i) = controllerLoad (1,i) + (packets * y);
                    controllerLoad (1,i) = controllerLoad (1,i) + (packets (1,j) * y);
%                     controllerCost = controllerCost + (fixedCost * x);
%                     totalCost = totalCost + (packets (1,j) * sp (i,j) * costPerUnit * y); %calculating the total cost
                    totalCost = totalCost + (fixedCost * x) + (packets (1,j) * switchController (r,j) * (pf (1,j) ^ (r-1)) * (1-pf (1,j)) * y); %calculating the total cost
%                     totalCost = totalCost + (packets * switchController (r,j) * (pf ^ (r-1)) * (1-pf) * y); %calculating the total cost
                end
%                 controllerLoad
            end    
        end
