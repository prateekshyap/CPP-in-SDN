function [ controllerLoad, totalCost, connections, sp, packets, totalLat] = capacitedRandomCost( controllerList, mat, n, alpha )
...
%   Detailed explanation goes here
    ...
    for i = 1 : noc %for each controller
        sp (i, 1:n) = bellmanFord (n, mat, controllerList (i)); %calculate the shortest paths
        connections = sp;
    end
...
%     fixedCost = 6500; %fixed cost for the controllers
    packets = randi ([500 700], 1, n); %in kilo requests per second
...
%     y = 0; %binary variable for switch connection
    ...
    for i = 1 : noc %for each controller
        for j = 1 : n %for each node
            ...
            if (p <= noc && j == controllerList (1,p)) %if the controller is not counted and j is equal to the controller
            ...
            if ((connections (i,j) ~= 0)&&(x == 0)) %if jth node is connected to the ith controller and jth node is not counted with x
                ...
                for k = 1 : noc %for each controller
                    if (j == controllerList (1,k)) %if jth node is a controller
...
%                 controllerLoad (1,i) = controllerLoad (1,i) + (packets (1,j) * sp (i,j) * costPerUnit * y);
                controllerLoad (1,i) = controllerLoad (1,i) + (packets (1,j) * sp (i,j) * y);
...
%                 totalCost = totalCost + (fixedCost * x) + (packets (1,j) * sp (i,j) * costPerUnit * y); %calculating the total cost
                totalCost = totalCost + (fixedCost * x) + (packets (1,j) * sp (i,j) * y); %calculating the total cost
            end
        end
    end
    for i = 1 : noc %for each controller
        if (controllerLoad (1,i) > (controllerCapacity*alpha)) %if the load is more than capacity
            controllerLoad (2,i) = 0; %store 0
        else
            controllerLoad (2,i) = 1; %store 1
        end
    end
    totalLat = avgLatency(connections, n, n, noc, controllerList);
%     ind = 0;
%     if noc ~= newNoc
%         ind = 1;
%         totalCost = totalCost - (fixedCost * (noc - newNoc));
%     end
end