function [ draftRet ] = drafting( noc, pop, member1, member2 )
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
    for k = 1 : noc %for each controller present in 1st member i.e. c1
        draftRet (1, k) = pop (member1, k); %insert all the nodes to draft
    end
    ...
    for j = 1 : noc %for each controller present in 2nd member i.e. c2
        ...
        for in = 1 : noc %for each controller present in draft
            if (draftRet (1,in) == pop (member2, j)) %if jth node is equal to the in'th node in the draft
                flag3 = 1; %update flag
                break; %break the loop to reject the node
            end
        end 
        if (flag3 == 0) %if the jth node is a new node
            draftRet (1, din) = pop (member2,j); %add it to the draft
