function [ totalCost ] = plannedControllerPlacement( controllers, mat, n, m )
...
%    fixedCost = 6500; %fixed cost for the controllers
    ...
    controllerCapacity = 5000; %in kilo requests for second
...
%                     totalCost = totalCost + (fixedCost * x) + (packets * sp (i,j) * costPerUnit * y); %calculating the total cost
                    totalCost = totalCost + (fixedCost * x) + (packets * switchController (r,j) * (pf ^ (r-1)) * (1-pf) * y); %calculating the total cost
                end
%                 controllerLoad
            end    
        end
