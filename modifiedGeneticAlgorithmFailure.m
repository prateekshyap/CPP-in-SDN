function [ bestFitness, bestFitnessF, bestFitnessF2, bestLocations, bestLoads, returnPop, optConn, totalLat ] = modifiedGeneticAlgorithmFailure( mat, n, ps, mi, noc )
global mutProb;
...
fitnessValuesF = zeros (1, ps); %an array to store the fitness values for each combination present in populations matrix for one failure
fitnessValuesF2 = zeros (1, ps); %an array to store the fitness values for each combination present in populations matrix for second failure
loads = zeros (ps, noc); %a matrix to store the loads on the controllers
bestIndex = 1; %stores the best fitness index
bestIndexF = 1; %stores the best fitness index for one failure
bestIndexF2 = 1; %stores the best fitness index for second failure
worstIndex = 1; %stores the worst fitness index
bestFitness = 0; %stores the best fitness value
bestFitnessF = 0; %stores the best fitness value for one failure
bestFitnessF2 = 0; %stores the best fitness value for second failure
...
populationsF = populations;
populationsF2 = populations;
...
%     [l, f, conn] = capacitedCost (populations (g,1:noc), mat, n); %storing the fitness value for the combination
    [l, f, lat, conn] = capacitedCostLatency(populations(g,1:noc), mat, n); %finding out the total cost of the network for fs
    [lF, fF, latF, connF] = costWithFailure(populationsF(g,1:noc), mat, n, 1); %finding out the total cost for 1 failure
    [lF3, fF2, latF2, connF2] = costWithFailure(populationsF2(g,1:noc), mat, n, 2); %finding out the total cost for second failure
    loads (g, 1:noc) = l(1,1:noc);
    fitnessValues(1,g) = f;
    fitnessValuesF(1,g) = fF;
    fitnessValuesF2(1,g) = fF2;
    conns (g,:,:) = conn;
    if (g > 1) %if we have more than one fitness values
        ...
        if (fitnessValuesF (1, g) < fitnessValuesF (1, bestIndex)) %if the newly calculated fitness value is less than the best fitness value
            bestIndexF = g; %update the best fitness
        end
        if (fitnessValuesF2 (1, g) < fitnessValuesF2 (1, bestIndex)) %if the newly calculated fitness value is less than the best fitness value
            bestIndexF2 = g; %update the best fitness
        end
...
%drafting
    ...
    draftF = drafting (noc, populationsF, c1, c2); %calling the drafting function
    draftF2 = drafting (noc, populationsF2, c1, c2); %calling the drafting function
%deletion
    [candidate, candidateF, candidateF2, fitnessVal, fitnessValF, fitnessValF2, optimalLoad, conn2] = optimalCandidatesFailure (draft, draftF, draftF2, noc, mat, n); %calling optimalCandidates function to find out the new candidate
%mutation
mutProb = mutProb + 1;
...
%     fprintf (' done\n');
    ...
    while (flag0 == 0) %if indicator is not updated
        for b = 1 : noc %for each controller present in the candidate
            if (candidateF (1,b) == randomController) %if the newly generated controller is already present
                randomController = randi ([1,n], 1, 1); %generate a new controller
                flag0 = 1; %update the flag
                break; %break the loop
            end
    ...
    candidateF (1,randomIndex) = randomController; %update the candidate
    ...
    while (flag0 == 0) %if indicator is not updated
        for b = 1 : noc %for each controller present in the candidate
            if (candidateF2 (1,b) == randomController) %if the newly generated controller is already present
                randomController = randi ([1,n], 1, 1); %generate a new controller
                flag0 = 1; %update the flag
                break; %break the loop
            end
    ...
    candidateF2 (1,randomIndex) = randomController; %update the candidate
...
%replacement
    ...
    indicator = 0; %a variable to keep track of the candidate
    for y = 1 : ps %for each population
        if (populationsF (y,1:noc) == candidateF (1,1:noc)) %if the candidate is equal to yth combination
            indicator = 1; %update the indicator
            break; %break the loop
        end
    end
    if (indicator == 0) %if indicator is not updated that means the candidate is new
        ...
        maxFitnessIndex = 1;
        for z = 2 : ps %for each population
            if (fitnessValuesF (1,z) > fitnessValuesF (1,maxFitnessIndex)) %if the new value is greater than the maximum fitness value
                maxFitnessIndex = z; %update the maximum fitness index
            end
        end
        if (fitnessValuesF (1,maxFitnessIndex) > fitnessValF) %if the maximum value is greater than the new value
            populationsF (maxFitnessIndex, 1:noc) = candidateF (1,1:noc); %replace the corresponding combination with the newly found candidate
            fitnessValuesF (1, maxFitnessIndex) = fitnessValF; %replace the corresponding fitness value with the newly found fitness value 
...
%             conns (maxFitnessIndex,:,:) = conn2; %replace the connections
    ...
    indicator = 0; %a variable to keep track of the candidate
    for y = 1 : ps %for each population
        if (populationsF2 (y,1:noc) == candidateF2 (1,1:noc)) %if the candidate is equal to yth combination
            indicator = 1; %update the indicator
            break; %break the loop
        end
    end
    if (indicator == 0) %if indicator is not updated that means the candidate is new
        ...
        maxFitnessIndex = 1;
        for z = 2 : ps %for each population
            if (fitnessValuesF2 (1,z) > fitnessValuesF2 (1,maxFitnessIndex)) %if the new value is greater than the maximum fitness value
                maxFitnessIndex = z; %update the maximum fitness index
            end
        end
        if (fitnessValuesF2 (1,maxFitnessIndex) > fitnessValF2) %if the maximum value is greater than the new value
            populationsF2 (maxFitnessIndex, 1:noc) = candidateF2 (1,1:noc); %replace the corresponding combination with the newly found candidate
            fitnessValuesF2 (1, maxFitnessIndex) = fitnessValF2; %replace the corresponding fitness value with the newly found fitness value 
%             loads (maxFitnessIndex, 1:noc) = optimalLoad; %replace the load values
%             conns (maxFitnessIndex,:,:) = conn2; %replace the connections
        end
    end
...
bestFitnessF = fitnessValuesF(1,bestIndexF);
bestFitnessF2 = fitnessValuesF2(1,bestIndexF2);
