function [ bestFitness, bestLocations, bestLoads ] = modifiedGeneticAlgorithmImpl05( mat, n, ps, mi, noc )
global mutProb;
...
%finding out the initial populations
populations = zeros (ps, noc); %a matrix to store all the populations
...
% finding out the fitness values, the best and the worst fitness values
for g = 1 : ps %for each population
%     populations (g, 1:noc) = randi (n,1,noc); %storing the combinations into populations matrix
    [l, f] = capacitedCost (populations (g,1:noc), mat, n); %storing the fitness value for the combination
    loads (g, 1:noc) = l;
    fitnessValues (1, g) = f;
...
fitnessValues (1,bestIndex)

%generation, drafting & deletion, mutation and replacement
