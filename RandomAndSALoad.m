...
for cont = 1 : maxNoc %for each number of controllers
    tic;
    [controllers, cost, xTemps, yCosts, xxIts, counter, lSa, randContm] = simulatedAnnealingL (mat, n, 10, 0.001, maxIterations, 0.95, cont); %perform simuated annealing
    cont
    cost
    lSa
    controllers
...
for cont = 1 : maxNoc %for each number of controllers
        ...
        [lRand, c] = capacitedCostL (randCont, mat, n); %finding out the cost
...
ylabel ('Cost');
xlabel ('Networks');
legend ('Random','SA','Location','BestOutside');
...
xlabel ('Temperature');
ylabel ('Fitness Function');
legend ('SA','Location','BestOutside');
