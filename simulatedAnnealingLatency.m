function [ fs, optLat, x, y, xx, counter, retRand ] = simulatedAnnealingLatency( mat, n, tMax, tMin, noi, cf, noc )
...
fs = randi (n,1,noc);
retRand = fs;
optLat = 1000000; %to store the optimal cost value
...
[l, lat] = capacitedCostLatency (fs, mat, n); %finding out the total cost of the network for fs
x (1,1) = t; %storing the first temperature
y (1,1) = lat; %storing the first cost
...
while (t > tMin) %current temperature should be more than minimum temperature
    ...
    for i = 1 : noi %for each iteration
        ...
        [l2, lat2] = capacitedCostLatency (fs2, mat, n); %cost for new combination fs2
        if (i == 1) %for the first iteration
            y (1,in) = lat2; %store the cost
        else %for next iterations
            if (lat2 < y (1,in)) %compare the new cost to the stored cost and if the new cost is less than the stored one
                y (1,in) = lat2; %update the cost
            end
        end
        delC = lat2 - lat; %cost difference
        if (lat2 <= lat) %if the new cost is smaller than previous one
            fs = fs2; %update the optimal positions of controllers
            optLat = lat2; %update the optimal cost
        else
