function [ fs, optCost, x, y1, xx, counter, retRand, totalLat] = simulatedAnnealingICIT( mat, n, tMax, tMin, noi, cf, noc )
...
[l, cost, conn] = capacitedCost1 (fs, mat, n); %finding out the total cost of the network for fs
...
while (t > tMin) %current temperature should be more than minimum temperature
    ...
    for i = 1 : noi %for each iteration
        ...
        [l2, cost2, conn2] = capacitedCost1 (fs2, mat, n); %cost for new combination fs2
...
totalLat = avgLatency(optConn, n, n, noc, optCost);
end